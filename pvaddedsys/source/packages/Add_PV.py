def addPV(dss, busName, bus, phase, load, voltage, pv_factor, pv_file, flag): # Add PV system to the circuit
    if flag == 0: # Write the header of the file, flag will be 0 only for the first time then these lines will not be executed
    ...
    pv_pmpp = load * pv_factor # Calculate the maximum power of the PV system
    pv_file.write(f"New PVSystem.PV{busName} phases={phase} bus1={bus} kv={voltage} irrad=0.98 pmpp={pv_pmpp} temperature=25 pf=1\n") # Write the PV system to the file
    pv_file.write("%cutin=0 %cutout=2 effcurve=MyEff P-tCurve=MyPvsT Daily=MyIrrad Tdaily=Mytemp\n\n\n") # Write the parameters of the PV system to the file


    # These lines are used to add a transformer to the PV systems but deprecated
